...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // 提示用户输入一个选项
            System.out.println("请输入一个数字（0-4）来选择执行的函数:");
            System.out.println("0: 展示有向图");
            System.out.println("1: 查询桥接词");
            System.out.println("2: 计算单词间最短路径");
            ...
            String word1, word2;
            // 根据用户输入执行不同的函数
            switch (Operation.values()[userInput]) {
                case showGraph:
                    // 展示有向图
                    System.out.println("展示有向图");
                    showDirectedGraph(graph);
                    break;
                case searchBridgeWords:
                    // 查询桥接词
    ...
    public static DirectedGraph buildGraph(String filePath) throws IOException {
    ...
    private static void showDirectedGraph(DirectedGraph graph) {
        ...
        try {
            Graphviz.fromGraph(g).width(GRAPH_WIDTH).render(Format.PNG).toFile(new File("graph.png"));
    ...
    public static List<String> queryBridgeWords(DirectedGraph graph, String word1, String word2) {
        ...
        if (possibleBridges.isEmpty()) {
            ...
            System.out.println(String.join(", ", possibleBridges) + ".");
    ...
    private static List<String> dijkstraShortestPath(DirectedGraph graph, String source, String target) {
        ...
        PriorityQueue<VertexDistance> priorityQueue =
                new PriorityQueue<>(Comparator.comparingInt(VertexDistance::getDistance));
        ...
        while (!priorityQueue.isEmpty()) {
            ...
            if (!visited.add(current.getVertex())) {
                continue;
            }

            for (Map.Entry<String, Integer> neighbor : graph.getNeighbors(current.getVertex()).entrySet()) {
                int newDist = distances.get(current.getVertex()) + neighbor.getValue();
                if (newDist < distances.get(neighbor.getKey())) {
                    distances.replace(neighbor.getKey(), newDist);
                    predecessors.put(neighbor.getKey(), current.getVertex());
    ...
    public static void visualizeAndHighlightPath(DirectedGraph graph, List<String> path, String dotFilename) {
        ...
        try (Writer writer = new OutputStreamWriter(new FileOutputStream(dotFilename), StandardCharsets.UTF_8)) {
...
//            process.waitFor();
    ...
    private static void writeToFile(List<String> nodes) {
        try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("randomWalk_result.txt"), StandardCharsets.UTF_8))) {
